...
class MotionGenConfig:
    ...
    def load_from_robot_config(
        ...
        js_particle_trajopt_file: Optional[str] = None,
        js_gradient_trajopt_file: Optional[str] = None,
        js_finetune_trajopt_file: Optional[str] = None,
        js_finetune_trajopt_iters: Optional[int] = None,
        js_grad_trajopt_iters: Optional[int] = None,
        ...
        js_trajopt_cfg = TrajOptSolverConfig.load_from_robot_config(
            ...
            world_coll_checker=world_coll_checker,
            base_cfg_file=base_config_data,
            particle_file=js_particle_trajopt_file,
            gradient_file=js_gradient_trajopt_file,
            ...
            self_collision_check=self_collision_check,
            self_collision_opt=self_collision_opt,
            grad_trajopt_iters=js_grad_trajopt_iters,
            interpolation_dt=interpolation_dt,
            use_particle_opt=trajopt_particle_opt,
            traj_evaluator_config=traj_evaluator_config,
            traj_evaluator=traj_evaluator,
        ...
        finetune_js_trajopt_cfg = TrajOptSolverConfig.load_from_robot_config(
            ...
            base_cfg_file=base_config_data,
            particle_file=js_particle_trajopt_file,
            gradient_file=js_finetune_trajopt_file,
            traj_tsteps=js_trajopt_tsteps,
            interpolation_type=interpolation_type,
            interpolation_steps=interpolation_steps,
            use_cuda_graph=use_cuda_graph,
            self_collision_check=self_collision_check,
            self_collision_opt=self_collision_opt,
            grad_trajopt_iters=js_finetune_trajopt_iters,
            interpolation_dt=interpolation_dt,
            use_particle_opt=False,
            traj_evaluator_config=traj_evaluator_config,
            traj_evaluator=traj_evaluator,
    ...
    def warmup(
        ...
        if batch is None:
            ...
            if n_goalset == -1:
                ...
                start_state.position[..., warmup_joint_index] += warmup_joint_delta
                weld_cost_metric = [WeldingCostMetric(
                            tool_axis_vec=torch.tensor([1.0, 1.0, 0.0], device=self.tensor_args.device),
                            welding_start_point=torch.tensor([0.5, 0.388, -0.049], device=self.tensor_args.device),
                            welding_end_point=torch.tensor([0.5, 0.388, 0.35], device=self.tensor_args.device),
                            welding_start_end_range=torch.tensor([0.3, 1.0], device=self.tensor_args.device),  # Assuming default range
                            welding_start_end_position_weight=torch.tensor([500000, 200000], device=self.tensor_args.device),
                            welding_tool_contact_weight=torch.tensor([200000000.0], device=self.tensor_args.device),
                            welding_tool_working_angle_weight=torch.tensor([100000.0, 5000000.0], device=self.tensor_args.device),
                            welding_movement_consistency_weight=torch.tensor([50000.0, 50000.0], device=self.tensor_args.device),
                            welding_start_end_angle=torch.tensor([-20.0, 20.0], device=self.tensor_args.device),
                            welding_start_end_angle_weight=torch.tensor([700000.0, 700000.0], device=self.tensor_args.device),
                            waypoint_position=torch.tensor([0.45, 0.35, 0.0], device=self.tensor_args.device),
                            waypoint_weight=torch.tensor([1000000.0], device=self.tensor_args.device),
                            waypoint_ratio=torch.tensor([0.25], device=self.tensor_args.device),
                            ),
                            WeldingCostMetric(
                            tool_axis_vec=torch.tensor([1, -1, 0], device=self.tensor_args.device),
                            welding_start_point=torch.tensor([0.5, -0.388, -0.049], device=self.tensor_args.device),
                            welding_end_point=torch.tensor([0.5, -0.388, 0.35], device=self.tensor_args.device),
                            welding_start_end_range=torch.tensor([0.3, 1.0], device=self.tensor_args.device),  # Assuming default range
                            welding_start_end_position_weight=torch.tensor([500000, 200000], device=self.tensor_args.device),
                            ...
                            WeldingCostMetric(
                            tool_axis_vec=torch.tensor([1, 0, -1], device=self.tensor_args.device),
                            welding_start_point=torch.tensor([0.5, 0.388, -0.049], device=self.tensor_args.device),
                            welding_end_point=torch.tensor([0.5, 0.0, -0.049], device=self.tensor_args.device),
                            welding_start_end_range=torch.tensor([0.3, 1.0], device=self.tensor_args.device),  # Assuming default range
                            welding_start_end_position_weight=torch.tensor([500000, 200000], device=self.tensor_args.device),
                            welding_tool_contact_weight=torch.tensor([200000000.0], device=self.tensor_args.device),
                            welding_tool_working_angle_weight=torch.tensor([100000.0, 5000000.0], device=self.tensor_args.device),
                ...
                for i in range(4):
                    self.plan_single(
                        ...
                        MotionGenPlanConfig(
                            ...
                            weld_cost_metric=weld_cost_metric[i%4],
                        ),
                        link_poses=link_poses,
                    )
                ...
                self.plan_single(
                    ...
                    MotionGenPlanConfig(
                        ...
                        parallel_finetune=parallel_finetune,
                        weld_cost_metric=weld_cost_metric[0],
                    ),
                    link_poses=link_poses,
                )
            else:
