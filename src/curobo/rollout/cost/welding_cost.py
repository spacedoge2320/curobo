...
def working_angle_consistency_error_fn(x_vectors:torch.Tensor, start_point: torch.Tensor, end_point:torch.Tensor, tool_axis_vec:torch.Tensor, cost_mask:torch.Tensor):
    ...
    consistency_error = torch.abs(projection_on_normal_weldline - projection_on_normal_weldline_sum/projection_on_normal_weldline_count_nonzero)

    consistency_error = torch.relu(torch.abs(consistency_error) - 0.008715574274)

    # Apply error for abs(projection on normal) > 0.08715574274
    bound_error = torch.relu(torch.abs(projection_on_normal_weldline) - 0.08715574274)
...
class WeldingCostMetric:
    ...
    welding_tool_working_angle_weight: torch.Tensor = torch.tensor([0.0, 0.0])
    ...
    def reset_metric(cls):
        return WeldingCostMetric( 
            ...
            welding_tool_working_angle_weight=torch.tensor([0.0, 0.0]),
...
class WeldingCost(CostBase):
    def __init__(self, config: CostConfig):
        ...
        with torch.no_grad():
            ...
            self.welding_tool_working_angle_weight = torch.tensor([0.0, 0.0], dtype=torch.float32, device=self.tensor_args.device)
    ...
    def forward(self, ee_pos_batch, ee_quat_batch):
        ...
        weldline_distance_cost =  torch.square(weldline_distance_error)*self.welding_tool_contact_weight
